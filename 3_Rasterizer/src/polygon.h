...
// I've changed it into a class with which I'm more confortable.
class Triangle
{
public:
...
public:
    ...
    // Compute Barycentric coordinates for 2D & 3D rasterizer
    glm::vec3 ComputeBaryCords2D(const Triangle &t, const glm::vec4 &p);
    glm::vec3 ComputeBaryCords3D(const glm::vec2 &v0, const glm::vec2 &v1, const glm::vec2 &v2, const glm::vec2 &p);

    // Triangulate concave polygons
    void EarClipTriangulate();

    // Helper functions for EarClipTriangulate
    int FindMeAConvexPoint();
    glm::vec4 ProjectToXY(glm::vec4 worldPos);
    bool isConvexPoint(glm::vec3 &frontNorm, std::vector<unsigned int> &currentList, int thisOne);
    bool isFeasible(std::vector<unsigned int> &currentList, int thisOne);
};
