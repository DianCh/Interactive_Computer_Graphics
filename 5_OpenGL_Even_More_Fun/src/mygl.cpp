...
void MyGL::resizeGL(int w, int h)
{
    ...
    for(std::shared_ptr<SurfaceShader> s : m_surfaceShaders)
    {
        s->setViewProjMatrix(m_camera.getView(), m_camera.getProj());
        s->setEyePos(glm::vec4(m_camera.eye, 1.f));
    }
...
void MyGL::createRenderBuffers()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, this->devicePixelRatio() * this->width(), this->devicePixelRatio() * this->height(), 0, GL_RGB, GL_UNSIGNED_BYTE, (void*)0);
...
void MyGL::createShaders()
{
    ...
    std::shared_ptr<PostProcessShader> tv = std::make_shared<PostProcessShader>(this);
    tv->create(":/glsl/post/passthrough.vert.glsl", ":/glsl/post/tvglitch.frag.glsl");
    m_postprocessShaders.push_back(tv);
