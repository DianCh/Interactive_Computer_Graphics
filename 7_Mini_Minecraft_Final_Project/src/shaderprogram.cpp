...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifSampler2D(-1), unifTime(-1), unifEyePos(-1), attrUVs(-1), unifDimensions(-1), attrFlags(-1),
      context(context), unifTickingFlag(-1), unifDeformWater(-1), unifDeformLava(-1)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    unifDimensions = context->glGetUniformLocation(prog, "u_Dimension");
    attrUVs        = context->glGetAttribLocation(prog, "vs_UV");
    attrFlags      = context->glGetAttribLocation(prog, "vs_Flag");

    unifTickingFlag= context->glGetUniformLocation(prog, "u_TickingFlag");
    unifDeformWater= context->glGetUniformLocation(prog, "u_DeformWater");
    unifDeformLava = context->glGetUniformLocation(prog, "u_DeformLava");
}
...
void ShaderProgram::setEyePos(glm::vec4 pos)
{
    useMe();

    if (unifEyePos != -1)
    {
        context->glUniform4fv(unifEyePos, 1, &pos[0]);
    }
}
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    // If so, it binds the appropriate buffers to each attribute.
      ...
      if (attrPos != -1 && attrNor != -1 && attrUVs != -1 && d.bindAttr()) {
          context->glEnableVertexAttribArray(attrPos);
          context->glVertexAttribPointer(attrPos, 4, GL_FLOAT, false, sizeof(glm::vec4)*3, NULL);

          context->glEnableVertexAttribArray(attrNor);
          context->glVertexAttribPointer(attrNor, 4, GL_FLOAT, false, sizeof(glm::vec4)*3, (GLvoid*)(sizeof(glm::vec4)));

          context->glEnableVertexAttribArray(attrUVs);
          context->glVertexAttribPointer(attrUVs, 4, GL_FLOAT, false, sizeof(glm::vec4)*3, (GLvoid*)(sizeof(glm::vec4)*2));
      }

      //=========================added by yuru========================
      if (attrPos != -1 && attrCol != -1 && d.bindAttr()) {
          context->glEnableVertexAttribArray(attrPos);
          context->glVertexAttribPointer(attrPos, 4, GL_FLOAT, false, sizeof(glm::vec4)*3, NULL);

          context->glEnableVertexAttribArray(attrCol);
          context->glVertexAttribPointer(attrCol, 4, GL_FLOAT, false, sizeof(glm::vec4)*3, (GLvoid*)(sizeof(glm::vec4)*2));
      }
      //==================================================================

      //=========================added by yuru========================
      if (attrPos != -1 && attrUVs != -1 && d.bindAttr()) {
          context->glEnableVertexAttribArray(attrPos);
          context->glVertexAttribPointer(attrPos, 4, GL_FLOAT, false, sizeof(glm::vec4)*3, NULL);
      ...
      if (attrFlags != -1 && d.bindFlag())
      {
          context->glEnableVertexAttribArray(attrFlags);
          context->glVertexAttribIPointer(attrFlags, 2, GL_INT, 0, NULL);
      }
    ...
    if (attrUVs != -1) context->glDisableVertexAttribArray(attrUVs);
    if (attrFlags != -1) context->glDisableVertexAttribArray(attrFlags);
...
void ShaderProgram::setTickingFlag(int f)
{
    useMe();

    if (unifTickingFlag != -1)
    {
        context->glUniform1i(unifTickingFlag, f);
    }
}

void ShaderProgram::setDeformWater(int dw)
{
    useMe();

    if (unifDeformWater != -1)
    {
        context->glUniform1i(unifDeformWater, dw);
    }
}

void ShaderProgram::setDeformLava(int dl)
{
    useMe();

    if (unifDeformLava != -1)
    {
        context->glUniform1i(unifDeformLava, dl);
    }
}
