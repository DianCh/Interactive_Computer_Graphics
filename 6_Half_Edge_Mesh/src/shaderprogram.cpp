...
ShaderProgram::ShaderProgram(GLWidget277 *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifColor(-1), unifBindMatrices(-1), unifTransMatrices(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrIDs = context->glGetAttribLocation(prog, "vs_IDs");            // New feature in hw8
    attrWeights = context->glGetAttribLocation(prog, "vs_Weights");    // New feature in hw8

    unifModel      = context->glGetUniformLocation(prog, "u_Model");
    unifModelInvTr = context->glGetUniformLocation(prog, "u_ModelInvTr");
    unifViewProj   = context->glGetUniformLocation(prog, "u_ViewProj");
    unifColor      = context->glGetUniformLocation(prog, "u_Color");
    unifBindMatrices = context->glGetUniformLocation(prog, "u_BindMatrices");   // New feature in hw8
    unifTransMatrices = context->glGetUniformLocation(prog, "u_TransMatrices"); // New feature in hw8
}
...
void ShaderProgram::setBindMatrices(const glm::mat4 binds[])
{
    useMe();

    if (unifBindMatrices != -1)
    {
        context->glUniformMatrix4fv(unifBindMatrices, 50, GL_FALSE, &(binds[0][0][0]));
    }
}

void ShaderProgram::setTransMatrices(const glm::mat4 trans[])
{
    useMe();

    if (unifTransMatrices != -1)
    {
        context->glUniformMatrix4fv(unifTransMatrices, 50, GL_FALSE, &(trans[0][0][0]));
    }
}
